;(function($,window,document,undefined){'use strict';var Timepicker=function(element,options){this.widget='';this.$element=$(element);this.defaultTime=options.defaultTime;this.disableFocus=options.disableFocus;this.isOpen=options.isOpen;this.minuteStep=options.minuteStep;this.modalBackdrop=options.modalBackdrop;this.secondStep=options.secondStep;this.showInputs=options.showInputs;this.showMeridian=options.showMeridian;this.showSeconds=options.showSeconds;this.template=options.template;this.appendWidgetTo=options.appendWidgetTo;this.minValue={hour:options.minValue.hour,minute:(options.minValue.minute===undefined?0:options.minValue.minute),second:(options.minValue.second===undefined?0:options.minValue.second),format:options.minValue.format};this.maxValue={hour:options.maxValue.hour,minute:(options.maxValue.minute===undefined?59:options.maxValue.minute),second:(options.maxValue.second===undefined?59:options.maxValue.second),format:options.maxValue.format};;this._init();};Timepicker.prototype={constructor:Timepicker,_init:function(){var self=this;if(this.$element.parent().hasClass('input-append')||this.$element.parent().hasClass('input-prepend')){this.$element.parent('.input-append, .input-prepend').find('.add-on').on({'click.timepicker':$.proxy(this.showWidget,this)});this.$element.on({'focus.timepicker':$.proxy(this.highlightUnit,this),'click.timepicker':$.proxy(this.highlightUnit,this),'keydown.timepicker':$.proxy(this.elementKeydown,this),'blur.timepicker':$.proxy(this.blurElement,this)});}else{if(this.template){this.$element.on({'focus.timepicker':$.proxy(this.showWidget,this),'click.timepicker':$.proxy(this.showWidget,this),'blur.timepicker':$.proxy(this.blurElement,this)});}else{this.$element.on({'focus.timepicker':$.proxy(this.highlightUnit,this),'click.timepicker':$.proxy(this.highlightUnit,this),'keydown.timepicker':$.proxy(this.elementKeydown,this),'blur.timepicker':$.proxy(this.blurElement,this)});}}
if(this.template!==false){this.$widget=$(this.getTemplate()).appendTo(this.$element.parents(this.appendWidgetTo)).on('click',$.proxy(this.widgetClick,this));}else{this.$widget=false;}
...
(this.showMeridian?'<td class="separator">&nbsp;</td>'+'<td><a href="#" data-action="toggleMeridian"><i class="icon-chevron-down"></i></a></td>':'')+'<tr id="timePickerMessageTr" style="display:none;"><td colspan=7><span class="text-error">ERROR<br />'+minValueText+'<br />'+maxValueText+' </span></td></tr>'+'</tr>'+'</table>';switch(this.template){case'modal':template='<div class="bootstrap-timepicker-widget modal hide fade in" data-backdrop="'+(this.modalBackdrop?'true':'false')+'">'+'<div class="modal-header">'+'<a href="#" class="close" data-dismiss="modal">×</a>'+'<h3>Pick a Time</h3>'+'</div>'+'<div class="modal-content">'+
...
this.$element.trigger({'type':'hide.timepicker','time':{'value':this.getTime(),'hours':this.hour,'minutes':this.minute,'seconds':this.second,'meridian':this.meridian}});if(this.template==='modal'){this.$widget.modal('hide');}else{this.$widget.removeClass('open');}
...
this.updateFromElementVal();if(this.template==='modal'){this.$widget.modal('show').on('hidden',$.proxy(this.hideWidget,this));}else{if(this.isOpen===false){this.$widget.addClass('open');}}
...
this.isValidTime();}else{this.$widget.find('span.bootstrap-timepicker-hour').text(hour);this.$widget.find('span.bootstrap-timepicker-minute').text(minute);if(this.showSeconds){this.$widget.find('span.bootstrap-timepicker-second').text(second);}
...
break;}},isValidTime:function(){var hourTmp=(this.hour<12&&this.meridian==='PM'?(this.hour+12):this.hour);hourTmp=(this.hour===12&&this.meridian==='AM'?0:hourTmp);var minHourTmp=(this.minValue.format==='PM'?(this.minValue.hour+12):this.minValue.hour);var maxHourTmp=(this.maxValue.format==='PM'?(this.maxValue.hour+12):this.maxValue.hour);var outRange=false;if(hourTmp>maxHourTmp){outRange=true;}
else if(hourTmp==maxHourTmp&&this.minute>this.maxValue.minute){outRange=true;}
else if(hourTmp==maxHourTmp&&this.minute==this.maxValue.minute&&this.second>this.maxValue.second){outRange=true;}
if(hourTmp<minHourTmp){outRange=true;}
else if(hourTmp==minHourTmp&&this.minute<this.minValue.minute){outRange=true;}
else if(hourTmp==minHourTmp&&this.minute==this.minValue.minute&&this.second<this.minValue.second){outRange=true;}
if(outRange){$('#timePickerMessageTr').show();}
else{$('#timePickerMessageTr').hide();}
return outRange;}};$.fn.timepicker=function(option){var args=Array.apply(null,arguments);args.shift();return this.each(function(){var $this=$(this),data=$this.data('timepicker'),options=typeof option==='object'&&option;if(!data){$this.data('timepicker',(data=new Timepicker(this,$.extend({},$.fn.timepicker.defaults,options,$(this).data()))));}
if(typeof option==='string'){data[option].apply(data,args);}});};$.fn.timepicker.defaults={defaultTime:'current',disableFocus:false,isOpen:false,minuteStep:15,modalBackdrop:false,secondStep:15,showSeconds:false,showInputs:true,showMeridian:true,template:'dropdown',appendWidgetTo:'.bootstrap-timepicker',minValue:{hour:0,minute:0,second:0},maxValue:{hour:23,minute:59,second:59}};$.fn.timepicker.Constructor=Timepicker;})(jQuery,window,document);